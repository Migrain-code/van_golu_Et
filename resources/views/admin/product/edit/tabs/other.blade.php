            ...
            @foreach($categories as $category)
                <optgroup label="{{$category->parent->parent->getName()." > ".$category->parent->getName()." > ".$category->getName()}}">
                    @foreach($category->series as $serie)
                        <option value="{{$serie->id}}" @selected($serie->id == $product->group_id)>{{$serie->getName()}}</option>
                    @endforeach
                </optgroup>
    ...
    <div class="d-flex flex-column mb-7 fv-row">
        ...
        <select name="reference_id[]" multiple id="product_select" aria-label="Referans Seçiniz"
            ...
            @foreach($references as $reference)
                <option value="{{$reference->id}}" @selected(in_array($reference->id, $referenceIds))>{{$reference->category->getName() ." > ".$reference->getName()}}</option>
            @endforeach
        </select>
    </div>
